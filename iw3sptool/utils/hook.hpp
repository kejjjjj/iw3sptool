...
#define PLUGIN_PREFIX "IW3SP_"
#define HOOK_PREFIX(x) (PLUGIN_PREFIX + std::string(x))
...
struct hookbase
{
	...
	constexpr const std::string& id() const noexcept { return s_id; }
	constexpr void* get_jmp() const noexcept { return backup; }
	void* detour = {};
...
struct hook : public hookbase
{
	...
	void add_callable(std::function<Type(Args...)>&& d) {
		functions.emplace_back(std::forward<std::function<Type(Args...)>&&>(d));
	}
...
struct hooktable
{
	...
	friend __declspec(dllexport) inline std::vector<phookbase>* gethooktable();
...
__declspec(dllexport) inline std::vector<phookbase>* gethooktable()
{
	return &hooktable::table;
}
